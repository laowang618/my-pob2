...
local NewTreeTabClass = newClass("NewTreeTab", "ControlHost", function(self, build)
	self.ControlHost()
	...
	self.viewer = new("NewPassiveTreeView")

	self.specList = { }
	self.specList[1] = new("NewPassiveSpec", build, latestTreeVersion)
	...
	self.controls.treeHeatMap = new("CheckBoxControl", { "LEFT", self.controls.treeSearch, "RIGHT" }, { 130, 0, 20 }, "Show Node Power:", function(state)
		...
		if state then
			self.controls.allocOrderSliderLabel:SetAnchor("LEFT", self.controls.powerReport, "RIGHT", nil, nil, nil)
			self.controls.allocOrderSliderLabel.shown = true
			self.controls.allocOrderSlider.shown = true
			self.controls.allocOrderSliderValue.shown = true
		else
			self.controls.allocOrderSliderLabel:SetAnchor("LEFT", self.controls.nodePowerMaxDepthSelect, "RIGHT", nil, nil, nil)
			self.controls.allocOrderSliderLabel.shown = false
			self.controls.allocOrderSlider.shown = false
			self.controls.allocOrderSliderValue.shown = false
		end
	...
	-- Allocation Order Slider
	self.controls.allocOrderSliderLabel = new("LabelControl", { "LEFT", self.controls.treeHeatMap, "RIGHT" }, { 8, 0, 150, 16 }, "^7Allocation Order:")
	self.controls.allocOrderSliderLabel.shown = false
	-- Set initial anchor to avoid overlap with nodePowerMaxDepthSelect
	self.controls.allocOrderSliderLabel:SetAnchor("LEFT", self.controls.nodePowerMaxDepthSelect, "RIGHT", nil, nil, nil)
	self.controls.allocOrderSlider = new("SliderControl", { "LEFT", self.controls.allocOrderSliderLabel, "RIGHT" }, { 8, 0, 120, 16 }, function(value)
		local allocCount = #(self.build.spec.allocOrderList or {})
		if allocCount > 0 then
			self.viewer.allocOrderLimit = m_floor(value * allocCount)
		else
			self.viewer.allocOrderLimit = 0
		end
		self.controls.allocOrderSliderValue.label = "^7" .. self.viewer.allocOrderLimit .. "/" .. allocCount
	end)
	self.controls.allocOrderSlider.shown = false
	self.controls.allocOrderSliderValue = new("LabelControl", { "LEFT", self.controls.allocOrderSlider, "RIGHT" }, { 8, 0, 0, 16 }, "^70")
	self.controls.allocOrderSliderValue.shown = false
...
function NewTreeTabClass:Draw(viewPort, inputEvents)
	self.anchorControls.x = viewPort.x + 4
	self.anchorControls.y = viewPort.y + viewPort.height - 24

	-- Update allocation order slider max value
	local allocCount = #(self.build.spec.allocOrderList or {})
	if allocCount > 0 then
		self.controls.allocOrderSlider.divCount = 1
		if self.viewer.allocOrderLimit == nil or self.viewer.allocOrderLimit > allocCount then
			self.viewer.allocOrderLimit = allocCount
		end
		self.controls.allocOrderSliderValue.label = "^7" .. self.viewer.allocOrderLimit .. "/" .. allocCount
	else
		self.controls.allocOrderSlider.divCount = 1
		if self.viewer.allocOrderLimit == nil then
			self.viewer.allocOrderLimit = 0
		end
		self.controls.allocOrderSliderValue.label = "^70/0"
	end
	...
	if viewPort.width >= widthSecondLineControls + rightMargin then
		...
		self.controls.powerReportList:SetAnchor("TOPLEFT", self.controls.treeHeatMap, "BOTTOMLEFT", 0, 4)
	else
...
function NewTreeTabClass:GetSpecList()
	local newSpecList = { }
...
function NewTreeTabClass:Load(xml, dbFileName)
	self.specList = { }
	if xml.elem == "Spec" then
		-- Import single spec from old build
		self.specList[1] = new("NewPassiveSpec", self.build, defaultTreeVersion)
	...
	for _, node in pairs(xml) do
		if type(node) == "table" then
			if node.elem == "Spec" then
				...
				local newSpec = new("NewPassiveSpec", self.build, node.attrib.treeVersion or defaultTreeVersion)
	...
	if not self.specList[1] then
		self.specList[1] = new("NewPassiveSpec", self.build, latestTreeVersion)
	end
	self:SetActiveSpec(tonumber(xml.attrib.activeSpec) or 1)
end

function NewTreeTabClass:PostLoad()
	for _, spec in ipairs(self.specList) do
		spec:PostLoad()
	end
end

function NewTreeTabClass:Save(xml)
	xml.attrib = {
...
function NewTreeTabClass:SetActiveSpec(specId)
...
function NewTreeTabClass:SetCompareSpec(specId)
...
function NewTreeTabClass:ConvertToVersion(version, remove, success, ignoreRuthlessCheck)
	...
	local newSpec = new("NewPassiveSpec", self.build, version)
...
function NewTreeTabClass:ConvertAllToVersion(version)
...
function NewTreeTabClass:OpenSpecManagePopup()
	local importTree =
...
function NewTreeTabClass:OpenVersionConvertPopup(version, ignoreRuthlessCheck)
	local controls = { }
...
function NewTreeTabClass:OpenVersionConvertAllPopup(version)
	local controls = { }
	controls.warningLabel = new("LabelControl", nil, {0, 20, 0, 16}, "^7Warning: some or all of the passives may be de-allocated due to changes in the tree.\n\n" ..
...
function NewTreeTabClass:OpenImportPopup()
	...
	local function decodePoePlannerTreeLink(treeLink)
		...
		local tmpSpec = new("NewPassiveSpec", self.build, latestTreeVersion)
		...
		local newSpec = new("NewPassiveSpec", self.build, newTreeVersion_or_errMsg)
	...
	local function decodeTreeLink(treeLink, newTreeVersion)
		...
		-- 20230908. We always create a new Spec()
		local newSpec = new("NewPassiveSpec", self.build, newTreeVersion)
		newSpec.title = controls.name.buf
...
function NewTreeTabClass:OpenExportPopup()
...
function NewTreeTabClass:ModifyAttributePopup(hoverNode)
	local controls = { }
...
function NewTreeTabClass:SaveMasteryPopup(node, listControl)
...
function NewTreeTabClass:OpenMasteryPopup(node, viewPort)
	local controls = { }
	local effects = { }
...
function NewTreeTabClass:SetPowerCalc(powerStat)
...
function NewTreeTabClass:BuildPowerReportList(currentStat)
	local report = {}
...
function NewTreeTabClass:FindTimelessJewel()
	local socketViewer = new("NewPassiveTreeView")
	...
	local function setupFallbackWeights()
		-- replaceHelperFunc is duplicated from NewPassiveSpec.lua
