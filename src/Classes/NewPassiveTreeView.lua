...
local NewPassiveTreeViewClass = newClass("NewPassiveTreeView", function(self)
	...
	self.hoverNode = nil
	self.allocOrderLimit = nil
end)

function NewPassiveTreeViewClass:Load(xml, fileName)
...
function NewPassiveTreeViewClass:Save(xml)
...
function NewPassiveTreeViewClass:Draw(build, viewPort, inputEvents)
	...
	for nodeId, node in pairs(spec.nodes) do
		...
		-- Check if node should be shown based on allocation order limit
		local allocOrder = spec.allocOrder[nodeId]
		local isAllocByOrder = (not allocOrder or not self.allocOrderLimit or allocOrder <= self.allocOrderLimit)
		local isAlloc = (node.alloc and isAllocByOrder) or build.calcsTab.mainEnv.grantedPassives[nodeId] or (compareNode and compareNode.alloc)
		SetDrawLayer(nil, 25)
		...
		if node and node.name ~= "Charm Socket" and node.containJewelSocket ~= true and (not node.expansionJewel or node.expansionJewel.size == 2) then
			...
			if node == hoverNode then
				...
				if isThreadOfHope then
					for _, radData in ipairs(build.data.jewelRadius) do
						local outerSize = radData.outer * data.gameConstants["NewPassiveTreeJewelDistanceMultiplier"] * scale
						local innerSize = radData.inner * data.gameConstants["NewPassiveTreeJewelDistanceMultiplier"] * scale
			...
			elseif node.alloc then
				if jewel and jewel.jewelRadiusIndex then
					...
					local outerSize = radData.outer * data.gameConstants["NewPassiveTreeJewelDistanceMultiplier"] * scale
					local innerSize = radData.inner * data.gameConstants["NewPassiveTreeJewelDistanceMultiplier"] * scale * 1.06
...
function NewPassiveTreeViewClass:DrawAsset(data, x, y, scale, isHalf)
...
function NewPassiveTreeViewClass:DrawImageRotated(handle, x, y, width, height, angle, ...)
...
function NewPassiveTreeViewClass:DrawQuadAndRotate(data, xTree, yTree, angleRad, treeToScreen)
	local vertActive = {}
		local xActive = xTree
		local yActive = yTree
...
function NewPassiveTreeViewClass:Zoom(level, viewPort)
...
function NewPassiveTreeViewClass:Focus(x, y, viewPort, build)
	self.zoomLevel = 20
	self.zoom = 1.2 ^ self.zoomLevel
...
function NewPassiveTreeViewClass:DoesNodeMatchSearchParams(node)
...
function NewPassiveTreeViewClass:AddNodeName(tooltip, node, build)
...
function NewPassiveTreeViewClass:AddNodeTooltip(tooltip, node, build, incSmallPassiveSkillEffect)
	local fontSizeBig = main.showFlavourText and 18 or 16
	tooltip.center = true
...
function NewPassiveTreeViewClass:IsConnectedToWeaponSetNodes(node)
...
function NewPassiveTreeViewClass:AddGlobalNodeWarningsToTooltip(tooltip, node, build)
	local isGlobalNode = node.type == "Keystone" or node.type == "Socket" or node.containJewelSocket
...
function NewPassiveTreeViewClass:DrawAllocMode(allocMode, viewPort)
	local rgbColor
	if allocMode == 0 then
		return
...
function NewPassiveTreeViewClass:LessLuminance()
