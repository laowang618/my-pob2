...
local NewPassiveSpecClass = newClass("NewPassiveSpec", "UndoHandler", function(self, build, treeVersion, convert)
	self.UndoHandler()
...
function NewPassiveSpecClass:Init(treeVersion, convert)
	...
	-- Record allocation order for each node
	-- Keys are node IDs, values are allocation order (1, 2, 3, ...)
	self.allocOrder = { }

	-- List of node IDs in allocation order
	self.allocOrderList = { }
end

function NewPassiveSpecClass:Load(xml, dbFileName)
	...
	if xml.attrib.nodes then
		...
		local allocOrderList = { }
		if xml.attrib.allocOrder then
			for nodeId in xml.attrib.allocOrder:gmatch("%d+") do
				t_insert(allocOrderList, tonumber(nodeId))
			end
		end
		for _, node in pairs(xml) do
			if type(node) == "table" then
				if node.elem == "Overrides" then
					for _, child in ipairs(node) do
						if child.elem == "AttributeOverride" then
							...
							ConPrintf("[NewPassiveSpecClass:Load] Unexpected element found in Overrides: " .. child.elem)
						end
					end
				end
			end
		end
		self:ImportFromNodeList(nil, classId, ascendClassId, tonumber(xml.attrib.secondaryAscendClassId or 0), hashList, weaponSets, copyTable(self.hashOverrides, true), masteryEffects, nil, allocOrderList)
	elseif url then
		self:DecodeURL(url)
	end
	self:ResetUndo()
...
function NewPassiveSpecClass:Save(xml)
	...
	local allocOrderList = { }
	for _, nodeId in ipairs(self.allocOrderList) do
		t_insert(allocOrderList, tostring(nodeId))
	end
	...
	xml.attrib = {
		...
		allocOrder = table.concat(allocOrderList, ",")
	}
	t_insert(xml, {
		-- Legacy format
		elem = "URL",
...
function NewPassiveSpecClass:PostLoad()
	self:BuildClusterJewelGraphs()
end

-- Import passive spec from the provided class IDs and node hash list
function NewPassiveSpecClass:ImportFromNodeList(className, classId, ascendClassId, secondaryAscendClassId, hashList, weaponSets, hashOverrides, masteryEffects, treeVersion, allocOrderList)
  if hashOverrides == nil then hashOverrides = {} end
  if allocOrderList == nil then allocOrderList = { } end
	...
	-- Load allocation order from saved data
	for i, nodeId in ipairs(allocOrderList) do
		if not self.allocOrder[nodeId] then
			self.allocOrder[nodeId] = i
			t_insert(self.allocOrderList, nodeId)
		end
	end
	...
	for _, id in pairs(hashList) do
		...
		if node then
			...
			if node.type ~= "Mastery" or (node.type == "Mastery" and self.masterySelections[id]) then
				...
				-- Record allocation order for imported nodes
				if not self.allocOrder[id] then
					self.allocOrder[id] = #self.allocOrderList + 1
					t_insert(self.allocOrderList, id)
				end
			end
		else
	...
	for _, id in pairs(self.allocExtendedNodes) do
		...
		if node then
			...
			node.allocMode = weaponSets[id] or 0
			self.allocNodes[id] = node
			-- Record allocation order for extended nodes
			if not self.allocOrder[id] then
				self.allocOrder[id] = #self.allocOrderList + 1
				t_insert(self.allocOrderList, id)
			end
...
function NewPassiveSpecClass:AllocateDecodedNodes(nodes, isCluster, endian)
...
function NewPassiveSpecClass:AllocateMasteryEffects(masteryEffects, endian)
...
function NewPassiveSpecClass:DecodePoePlannerURL(url, return_tree_version_only)
...
function NewPassiveSpecClass:DecodeURL(url)
	local b = common.base64.decode(url:gsub("^.+/",""):gsub("-","+"):gsub("_","/"))
...
function NewPassiveSpecClass:EncodeURL(prefix)
...
function NewPassiveSpecClass:SelectClass(classId)
...
function NewPassiveSpecClass:ResetAscendClass()
...
function NewPassiveSpecClass:SelectAscendClass(ascendClassId)
	self:ResetAscendClass()
...
function NewPassiveSpecClass:SelectSecondaryAscendClass(ascendClassId)
...
function NewPassiveSpecClass:IsClassConnected(classId)
...
function NewPassiveSpecClass:ConnectToClass(classId)
...
function NewPassiveSpecClass:ResetNodes()
	for id, node in pairs(self.nodes) do
	...
	wipeTable(self.masterySelections)
	-- Clear allocation order tracking
	wipeTable(self.allocOrder)
	wipeTable(self.allocOrderList)
end
...
function NewPassiveSpecClass:AllocNode(node, altPath)
	...
	if #node.intuitiveLeapLikesAffecting > 0 then
		...
		-- Record allocation order if not already recorded
		if not self.allocOrder[node.id] then
			self.allocOrder[node.id] = #self.allocOrderList + 1
			t_insert(self.allocOrderList, node.id)
		end
	else
		for _, pathNode in ipairs(altPath or node.path) do
			...
			-- Record allocation order if not already recorded
			if not self.allocOrder[pathNode.id] then
				self.allocOrder[pathNode.id] = #self.allocOrderList + 1
				t_insert(self.allocOrderList, pathNode.id)
			end
		end
	end
...
function NewPassiveSpecClass:DeallocSingleNode(node)
	node.alloc = false
	node.allocMode = 0
	self.allocNodes[node.id] = nil
	-- Remove from allocation order tracking
	if self.allocOrder[node.id] then
		self.allocOrder[node.id] = nil
		for i, id in ipairs(self.allocOrderList) do
			if id == node.id then
				t_remove(self.allocOrderList, i)
				break
			end
		end
	end
...
function NewPassiveSpecClass:DeallocNode(node)
...
function NewPassiveSpecClass:CountAllocNodes()
...
function NewPassiveSpecClass:FindStartFromNode(node, visited, noAscend)
...
function NewPassiveSpecClass:GetJewel(itemId)
...
function NewPassiveSpecClass:BuildPathFromNode(root)
	root.pathDist = 0
	root.path = { }
	local queue = { root }
...
function NewPassiveSpecClass:SetNodeDistanceToClassStart(root)
...
function NewPassiveSpecClass:AddMasteryEffectOptionsToNode(node)
	node.sd = {}
...
function NewPassiveSpecClass:NodesInIntuitiveLeapLikeRadius(node)
	local result = { }
...
function NewPassiveSpecClass:BuildAllDependsAndPaths()
...
function NewPassiveSpecClass:ReplaceNode(old, newNode)
...
function NewPassiveSpecClass:ReconnectNodeToClassStart(node)
...
function NewPassiveSpecClass:BuildClusterJewelGraphs()
...
function NewPassiveSpecClass:BuildSubgraph(jewel, parentSocket, id, upSize, importedNodes, importedGroups)
...
function NewPassiveSpecClass:CreateUndoState()
	local allocNodeIdList = { }
	local weaponSets = { }
	for nodeId in pairs(self.allocNodes) do
		t_insert(allocNodeIdList, nodeId)
...
function NewPassiveSpecClass:RestoreUndoState(state, treeVersion)
...
function NewPassiveSpecClass:SetWindowTitleWithBuildClass()
...
function NewPassiveSpecClass:NodeAdditionOrReplacementFromString(node,sd,replacement)
	local addition = {}
	addition.sd = {sd}
	addition.mods = { }
...
function NewPassiveSpecClass:NodeInKeystoneRadius(keystoneNames, nodeId, radiusIndex)
	for _, node in pairs(self.nodes) do
...
function NewPassiveSpecClass:SwitchAttributeNode(nodeId, attributeIndex)
