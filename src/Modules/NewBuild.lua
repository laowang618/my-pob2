...
local newBuildMode = new("ControlHost")
...
function newBuildMode:Init(dbFileName, buildName, buildXML, convertBuild, importLink)
	...
	self.newTreeTab = new("NewTreeTab", self)
	self.skillsTab = new("SkillsTab", self)
	self.calcsTab = new("CalcsTab", self)
	-- Set the spec reference to the newTreeTab's spec
	self.spec = self.newTreeTab.spec
...
function newBuildMode:SyncLoadouts()
	...
	if self.newTreeTab ~= nil and self.itemsTab ~= nil and self.skillsTab ~= nil and self.configTab ~= nil then
		local transferTable = {}
		local sortedTreeListSpecialLinks = {}
		for id, spec in ipairs(self.newTreeTab.specList) do
	...
	if self.newTreeTab then
		local spec = self.newTreeTab.specList[self.newTreeTab.activeSpec]
...
function newBuildMode:EstimatePlayerProgress()
...
function newBuildMode:CanExit(mode)
...
function newBuildMode:Shutdown()
...
function newBuildMode:Save(xml)
	xml.attrib = {
...
function newBuildMode:ResetModFlags()
	self.modFlag = false
	self.notesTab.modFlag = false
	self.partyTab.modFlag = false
	self.configTab.modFlag = false
	self.newTreeTab.modFlag = false
	self.newTreeTab.searchFlag = false
...
function newBuildMode:OnFrame(inputEvents)
	...
	elseif self.viewMode == "TREE" then
		self.newTreeTab:Draw(tabViewPort, inputEvents)
	...
	self.unsaved = self.modFlag or self.notesTab.modFlag or self.partyTab.modFlag or self.configTab.modFlag or self.newTreeTab.modFlag or self.newTreeTab.searchFlag or self.spec.modFlag or self.skillsTab.modFlag or self.itemsTab.modFlag or self.calcsTab.modFlag
...
function newBuildMode:UpdateClassDropdowns(treeVersion)
...
function newBuildMode:OpenConversionPopup()
	local controls = { }
...
function newBuildMode:OpenSavePopup(mode)
	local modeDesc = {
		["LIST"] = "now?",
		["EXIT"] = "before exiting?",
...
function newBuildMode:OpenSaveAsPopup()
...
function newBuildMode:OpenSpectreLibrary(library)
...
function newBuildMode:OpenSimilarPopup()
	local controls = { }
...
function newBuildMode:RefreshSkillSelectControls(controls, mainGroup, suffix)
...
function newBuildMode:FormatStat(statData, statVal, overCapStatVal, colorOverride)
...
function newBuildMode:AddDisplayStatList(statList, actor)
...
function newBuildMode:InsertItemWarnings()
...
function newBuildMode:RefreshStatList()
	self.controls.warnings.lines = {}
...
function newBuildMode:CompareStatList(tooltip, statList, actor, baseOutput, compareOutput, header, nodeCount)
	local count = 0
...
function newBuildMode:AddStatComparesToTooltip(tooltip, baseOutput, compareOutput, header, nodeCount)
	local count = 0
...
do
	local req = { }
	function newBuildMode:AddRequirementsToTooltip(tooltip, level, str, dex, int, strBase, dexBase, intBase)
...
function newBuildMode:LoadDB(xmlText, fileName)
	-- Parse the XML
...
function newBuildMode:LoadDBFile()
...
function newBuildMode:SaveDB(fileName)
...
function newBuildMode:SaveDBFile()
	if not self.dbFileName then
		self:OpenSaveAsPopup()
		return
	end
